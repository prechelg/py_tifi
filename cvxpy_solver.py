"""
CVXPY Solver Module for Fast Ion Distribution Tomography

This module provides convex optimization solvers for reconstructing fast ion distributions
from diagnostic measurements. It uses CVXPY to solve regularized inverse problems with
...
Usage:
    from cvxpy_solver import solve_distribution, solve_distribution_parallel
...
def solve_distribution(
    ...
    **solver_kwargs
    ...
    solver : str, optional
        CVXPY solver to use (e.g., 'SCS', 'ECOS', 'MOSEK', 'CLARABEL')
        If None, uses CVXPY default
    verbose : bool
    ...
    **solver_kwargs : dict
    ...
    try:
        ...
        warnings.warn(f"Solver failed: {e}. Trying with default solver.")
...
def solve_distribution_advanced(
    ...
    **solver_kwargs : dict
    ...
    Example:
    --------
    >>> # Example 1: Two diagnostics with float alpha
    ...
    ...     loss_locations=[(90e3, -0.7)]
    ... )  # Shape: (1640, 1641) = 1640 SD + 1 loss
    >>>
    ...
    ... )
    >>>
    >>> # Example 2: Three diagnostics with list of weights
    >>> A_NPA = ...  # Third diagnostic
    >>> result = solve_distribution_advanced(
    ...     A_diagnostics=[A_FIDA, A_SSNPA, A_NPA],
    ...     Phi=Phi_combined,
    ...     signals=[y_FIDA, y_SSNPA, y_NPA],
    ...     num_slowing_down=1640,
    ...     alpha=[0.6, 0.3, 0.1],  # 60% FIDA, 30% SSNPA, 10% NPA
    ...     lambda_sd=5e-6,
    ...     lambda_ext=2.5e-3
    ... )
    """
    ...
    try:
        ...
        warnings.warn(f"Solver failed: {e}. Trying default solver.")
        problem.solve(verbose=verbose)
...
def grid_search_hyperparameters(
    ...
    **kwargs
    ...
    """
    Parallel grid search over all three hyperparameters.
    ...
    **kwargs : dict
    ...
    >>>
    >>> # Grid search: 2 × 4 × 4 = 32 combinations
    ...
    ... )
    >>>
    >>> # Find best by residual
    >>> best_params = min(results.keys(), key=lambda k: results[k]['residual'])
    >>> print(f"Best: alpha={best_params[0]}, λ_sd={best_params[1]:.1e}, λ_ext={best_params[2]:.1e}")
    ...
    def solve_single_combo(params):
        ...
        try:
            ...
            return params, {'status': f'FAILED: {e}', 'residual': np.inf}
...
if __name__ == "__main__":
    ...
    print("CVXPY Tomography Solver Module")
    print("=" * 70)
    print()

    # Create synthetic problem
    print("Creating synthetic tomography problem...")
    ...
    try:
        ...
        # Define grids
        E_values = np.linspace(1e3, 120e3, 30)  # Smaller for demo
        p_values = np.linspace(-0.99, 0.99, 31)

        print(f"Energy grid: {len(E_values)} points")
        print(f"Pitch grid: {len(p_values)} points")
        print(f"Total grid points: {len(E_values) * len(p_values)}")
        print()

        # Create basis matrix
        print("Generating basis matrix...")
        ...
        A = create_fida_diagnostic(E_values, p_values, [5, 25, 45, 65, 85])
        print(f"Diagnostic matrix shape: {A.shape}")
        print()

        # Create true distribution (linear combination of 3 basis functions)
        print("Generating synthetic signal...")
        true_coeffs = np.zeros(Phi.shape[1])

        # Add 3 basis functions
        E0_list = [100e3, 50e3, 33e3]
        p0_list = [-0.7, -0.7, -0.7]
        weights = [0.5, 1.0, 1.0]

        for E0, p0, weight in zip(E0_list, p0_list, weights):
            i = np.argmin(np.abs(E_values - E0))
            j = np.argmin(np.abs(p_values - p0))
            col_idx = i * len(p_values) + j
            true_coeffs[col_idx] = weight

        true_dist = Phi @ true_coeffs
        signal = generate_signal(A, true_dist, noise_level=0.05, random_seed=42)
        print(f"Signal shape: {signal.shape}")
        print(f"Signal range: [{signal.min():.2e}, {signal.max():.2e}]")
        print()

        # Solve with L1 regularization
        print("Solving with L1 regularization...")
        result_l1 = solve_distribution(
            A, Phi, signal,
            lambda_reg=1e-3,
            regularization='l1',
            verbose=False
        )
        print(f"Status: {result_l1['status']}")
        print(f"Residual: {result_l1['residual']:.2e}")
        print(f"Objective: {result_l1['objective']:.2e}")
        print(f"Solve time: {result_l1['solve_time']:.3f}s")

        # Check sparsity
        indices, values = extract_sparse_coefficients(result_l1['coefficients'], threshold=1e-2)
        print(f"Non-zero coefficients: {len(indices)} out of {len(result_l1['coefficients'])}")
        print()

        # Compare regularizations
        print("Comparing different regularization methods...")
        comparison = compare_regularizations(A, Phi, signal, lambda_reg=1e-3, verbose=True)
        print()

        # Parallel solve for multiple lambdas
        print("Testing parallel solver with multiple lambda values...")
        lambda_values = np.logspace(-5, -2, 5)
        results = solve_distribution_parallel(
            A, Phi, signal, lambda_values, n_jobs=1  # Use 1 for demo
        )
        print(f"Solved for {len(results)} lambda values")

        # Find optimal lambda
        optimal_lambda = find_optimal_lambda(results, method='l_curve')
        print(f"Optimal lambda (L-curve): {optimal_lambda:.2e}")
        print(f"Optimal residual: {results[optimal_lambda]['residual']:.2e}")
        print()

        print("Example complete!")
        print()
        print("For plotting results, use:")
        print("  import matplotlib.pyplot as plt")
        print("  dist_2d = result['distribution'].reshape(len(p_values), len(E_values))")
        print("  plt.imshow(dist_2d, aspect='auto', cmap='hot_r', origin='lower')")
        print("  plt.show()")

    except ImportError as e:
        print(f"Skipping full example: {e}")
        print("Install required modules: slowing_down_basis, diagnostics")
        print()
        print("Basic solver demonstration...")

        # Create simple synthetic problem
        np.random.seed(42)
        n_measurements = 100
        n_grid = 50
        n_basis = 50

        A = np.random.randn(n_measurements, n_grid) * 0.1
        Phi = np.random.randn(n_grid, n_basis) * 0.1
        Phi = np.maximum(Phi, 0)  # Make basis non-negative

        true_c = np.zeros(n_basis)
        true_c[[5, 15, 25]] = [1.0, 0.5, 0.8]

        signal = A @ Phi @ true_c + 0.01 * np.random.randn(n_measurements)

        result = solve_distribution(A, Phi, signal, lambda_reg=1e-2, regularization='l1')
        print(f"Status: {result['status']}")
        print(f"Residual: {result['residual']:.2e}")

        indices, values = extract_sparse_coefficients(result['coefficients'])
        print(f"Non-zero coefficients: {len(indices)}")
