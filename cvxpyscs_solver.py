"""
CVXPY-SCS GPU Solver Module for Fast Ion Distribution Tomography

This module provides convex optimization solvers using the SCS (Splitting Conic Solver)
backend with GPU acceleration. It maintains the same API as cvxpy_solver.py but is
optimized for GPU performance.

SCS is particularly well-suited for:
- Large-scale conic programs
...
Usage:
    from cvxpyscs_solver import solve_distribution, solve_distribution_parallel

    # Single solve with GPU
    result = solve_distribution(A, Phi, signal, lambda_reg=1e-3, use_gpu=True)

    # Parallel solve with GPU
    results = solve_distribution_parallel(A, Phi, signal, lambda_values=[1e-4, 1e-3, 1e-2], use_gpu=True)

Requirements:
    pip install cvxpy
    pip install scs[gpu]  # For GPU support
"""
...
def get_scs_kwargs(use_gpu: bool = True, **user_kwargs) -> Dict:
    """
    Get default SCS solver parameters optimized for tomography problems.

    Parameters:
    -----------
    use_gpu : bool
        If True, enable GPU acceleration (requires scs[gpu])
    **user_kwargs : dict
        User-provided overrides for SCS parameters

    Returns:
    --------
    scs_kwargs : dict
        Dictionary of SCS parameters
    """
    # Default SCS parameters optimized for tomography
    defaults = {
        'eps_abs': 1e-6,        # Absolute tolerance
        'eps_rel': 1e-6,        # Relative tolerance
        'max_iters': 10000,     # Maximum iterations
        'acceleration_lookback': 10,  # Acceleration parameter
        'use_indirect': False,  # Use direct method (better for medium problems)
    }

    # GPU-specific settings
    if use_gpu:
        defaults['gpu'] = True
        defaults['use_indirect'] = True  # GPU requires indirect solver
...
def solve_distribution(
    ...
    **solver_kwargs
    ...
    """
    Solve the inverse problem using SCS solver with optional GPU acceleration.
    ...
    use_gpu : bool
        If True, use GPU acceleration (requires scs[gpu], default: True)
    verbose : bool
        If True, print solver output (default: False)
    **solver_kwargs : dict
        Additional arguments passed to SCS solver (overrides defaults)
    ...
    # Get SCS solver kwargs
    scs_kwargs = get_scs_kwargs(use_gpu=use_gpu, **solver_kwargs)

    try:
        problem.solve(solver=cp.SCS, verbose=verbose, **scs_kwargs)
    except cp.error.SolverError as e:
        warnings.warn(f"SCS solver failed: {e}. Trying without GPU.")
        # Fallback to CPU if GPU fails
        scs_kwargs_cpu = get_scs_kwargs(use_gpu=False, **solver_kwargs)
        problem.solve(solver=cp.SCS, verbose=verbose, **scs_kwargs_cpu)
...
def solve_distribution_parallel(
    ...
    **kwargs : dict
        Additional arguments passed to solve_distribution (including use_gpu)
...
def compute_l_curve(
    ...
    Example:
    --------
    >>> results = solve_distribution_parallel(A, Phi, signal, lambda_values, use_gpu=True)
...
def compare_regularizations(
    ...
    **kwargs : dict
        Additional arguments (including use_gpu)
    ...
    Example:
    --------
    >>> comparison = compare_regularizations(A, Phi, signal, lambda_reg=1e-3, use_gpu=True)
    >>> # Compare sparsity
...
def solve_distribution_advanced(
    ...
    **solver_kwargs : dict
    ...
    try:
        ...
        warnings.warn(f"SCS solver failed: {e}. Trying CPU fallback.")
        scs_kwargs_cpu = get_scs_kwargs(use_gpu=False, **solver_kwargs)
        problem.solve(solver=cp.SCS, verbose=verbose, **scs_kwargs_cpu)
...
def grid_search_hyperparameters(
    ...
    **kwargs
    ...
    """
    Parallel grid search over all three hyperparameters using SCS solver.
    ...
    **kwargs : dict
        Additional arguments for solve_distribution_advanced (including use_gpu)
    ...
    print(f"Grid search (SCS): {len(param_combinations)} combinations")
    ...
    def solve_single_combo(params):
        ...
        try:
            ...
            return params, {'status': f'FAILED: {e}', 'residual': np.inf, 'solver': 'SCS'}
...
if __name__ == "__main__":
    ...
    print("CVXPY-SCS GPU Tomography Solver Module")
    print("=" * 70)
    print()
    print("This module uses the SCS solver with GPU acceleration for")
    print("fast convex optimization of tomography inverse problems.")
    print()

    # Check if SCS is available
    try:
        import scs
        print(f"✓ SCS version {scs.__version__} installed")

        # Try to check for GPU support
        try:
            # This is a heuristic - SCS doesn't have a simple API to check GPU availability
            print("  To verify GPU support, check SCS installation:")
            print("  pip show scs")
            print("  Look for 'scs[gpu]' in the package info")
        except:
            pass

    except ImportError:
        print("✗ SCS not installed")
        print("  Install with: pip install scs")
        print("  For GPU: pip install scs[gpu]")

    print()
    print("Example usage:")
    print("  from cvxpyscs_solver import solve_distribution")
    print("  result = solve_distribution(A, Phi, signal, lambda_reg=1e-3, use_gpu=True)")
    print()
    print("For detailed examples, see test_cvxpy_solver.py")
